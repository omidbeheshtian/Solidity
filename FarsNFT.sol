...
contract ERC721OmidB is ERC721Full, ReentrancyGuard {
    ...
    */
    ...
    constructor(address _owner, address payable _admin, uint256 _commissionRate, string memory name, string memory symbol, bool _anyoneCanMint) public 
        ...
        require(_commissionRate<=100, "ERC721OmidB: Commission rate has to be between 0 and 100");
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        // onlyOwner
        require(ownerOf(tokenId)==msg.sender, "ERC721OmidB: Only owner can sell this item");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "ERC721OmidB: Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) public payable nonReentrant {
        ...
        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "ERC721OmidB: The collectible is not for sale");

        // transfer funds
        require(msg.value >= sellBidPrice[tokenId], "ERC721OmidB: Not enough funds");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "ERC721OmidB: The seller cannot buy his own collectible");
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {
        ...
        require(sellBidPrice[tokenId]==0, "ERC721OmidB: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "ERC721OmidB: The selected NFT already has an auction");
        require(ownerOf(tokenId)==msg.sender, "ERC721OmidB: Only owner can auction this item");
    ...
    function bid(uint256 tokenId) public payable nonReentrant {
        ...
        address owner = ownerOf(tokenId);
        require(msg.sender!=owner, "ERC721OmidB: The owner cannot bid his own collectible");
